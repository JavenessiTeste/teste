...
if($dados['T']=='FCD'){
	...
	$sql = "select * from ESP_GETNET where aux_busca = ".aspas($dados['ID']);
	$res  = jn_query($sql);
...
if($dados['T']=='CC'){
	...
	$resultadoPagamentoCredito =  PagamentoCredito($valor,$dados['FAT'].'-'.$dados['REG'],$codigoAssociado,$nome,$email,$documento,$dados['CT'],$endereco,$numero,$complemento,$bairro,$cidade,$estado,$cep,$dados['CTC'],$dados['CB'],$dados['CN'],$dados['CE'][0],$dados['CE'][1],$dados['CV'],$chave);
	...
	if($resultadoPagamentoCredito['status_code']== 200){
		if($resultadoPagamentoCredito['status'] == 'APPROVED'){
			...
			efetuaBaixa('CC',$dados['FAT'],$dados['REG'],$valor,$resultadoPagamentoCredito['credit']['terminal_nsu']);
		}
	}
...
if($resultadoPagamentoDebito['redirect_url']!= ''){
	echo '<form enctype="application/x-www-form-urlencoded" method="POST" action="'.$resultadoPagamentoDebito['redirect_url'].'" name="FormBD" id="FormBD" target="_blank">
				...
				<input type="hidden" name="TermUrl" id="" value="https://app.plenasaude.com.br/AliancaNet/services/retornoPagamentoDebito.php?ID='.$resultadoPagamentoDebito['payment_id'].'" />
		   </form>';
