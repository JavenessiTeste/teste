...
if($dadosInput['tipo'] =='carregaDados'){
	...
	$query .= ' MES_REFERENCIA, INFORMACOES_GERACAO FROM VW_REATIVACAO_AL2 ';
	...
	while ($row = jn_fetch_object($res)){		
		...
		$dadoLinha['CHECK'] = 'S';		
		$dadosGrid[] = $dadoLinha;		
		$retorno['TP_GERACAO']  = jn_utf8_encode($row->INFORMACOES_GERACAO);
	}
	...
	if($quantFaturas > 1){
		...
		foreach($dadosInput['faturas'] as $faturas){
			$selectFatura = "Select NUMERO_REGISTRO from VW_REATIVACAO_AL2 where 1=1 ";			
		...
		if($erro == ""){
			$retorno['STATUS'] = 'OK';
			$retorno['LINK'] = 'https://app.plenasaude.com.br/AliancaNet/services/efetuarPagamento.php?tipo=FA&info=REAT&id='.$identificacao;			
		}else{
			$retorno['STATUS'] = 'ERRO';
			$retorno['MSG'] = $erro;		
		}	
		...
		$retorno['STATUS'] = 'OK';
		$retorno['LINK'] = 'https://app.plenasaude.com.br/AliancaNet/services/efetuarPagamento.php?tipo=F&info=REAT&reg='.$dadosInput["faturas"][0];			
	}
