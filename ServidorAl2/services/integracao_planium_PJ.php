...
function salvaEmpresa(){
	...
	$insertEmpresa .= ' 	CODIGO_TIPO_CONTRATACAO, TIPO_EMPRESA, CODIGO_TIPO_CARACTERISTICA, CODIGO_GRUPO_FATURAMENTO, ';
	$insertEmpresa .= ' 	CODIGO_SITUACAO_ATENDIMENTO, CODIGO_GRUPO_PESSOAS, ';
	...
	$insertEmpresa .= aspas($rowParametros->CODIGO_TIPO_CONTRATACAO) . ',';
	$insertEmpresa .= aspas($rowParametros->TIPO_EMPRESA) . ',';
	$insertEmpresa .= aspasNull($rowParametros->CODIGO_CARACTERISTICA_EMPRESA) . ',';
	$insertEmpresa .= aspasNull($rowParametros->CODIGO_GRUPO_FATURAMENTO) . ',';
	$insertEmpresa .= aspasNull($rowParametros->CODIGO_SITUACAO_ATENDIMENTO) . ',';
	$insertEmpresa .= aspasNull($rowParametros->CODIGO_GRUPO_PESSOAS) . ',';
...
function salvaContrato(){
	...
	if(retornaValorConfiguracao('VALIDACOES_ESP_PLANIUM')){
		$queryDiaVenc = " SELECT TOP 1 DIA_VENCIMENTO FROM ESP0040 WHERE CODIGO_PLANO = '122' AND DIA_CADASTRO = CONVERT(date, GETDATE())";
		$resDiaVenc = jn_query($queryDiaVenc);
		$rowDiaVenc = jn_fetch_object($resDiaVenc);
		
		$diaVencimento = $rowDiaVenc->DIA_VENCIMENTO;
	}
...
function salvaPlanosEmpresa(){
	...
	$queryPlanosEmpresa  = 'INSERT INTO TMP1059_NET ( ';
	$queryPlanosEmpresa .= ' CODIGO_EMPRESA, CODIGO_PLANO, CODIGO_TABELA_PRECO) ';	
	$queryPlanosEmpresa .= 'VALUES ( ';	
	$queryPlanosEmpresa .= aspas($codigoEmpresa) . ", ";	
	$queryPlanosEmpresa .= aspas($rowParametros->CODIGO_PLANO) . ", ";	
	$queryPlanosEmpresa .= aspas($rowParametros->CODIGO_TABELA_PRECO);
	$queryPlanosEmpresa .= " ) ";	
	
	if (! jn_query($queryPlanosEmpresa)) {
		$retorno .= 'Erro: Nao foi possivel gravar a tabela TMP1059_NET ';
		return false; // saio retornando false
	}
}
