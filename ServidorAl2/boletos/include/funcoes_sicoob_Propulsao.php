...
$dadosboleto["dv_nosso_numero"] = digitoVerificador_NossoNumero($agencia . formata_numero($convenio,10,0) . $dadosboleto["nosso_numero"]);
$dvnossonumero = $dadosboleto["dv_nosso_numero"];

$campolivre  = "$modalidadecobranca$convenio$nossonumero$dvnossonumero$numeroparcela";

$dv='*';

$linha="$codigobanco$nummoeda$dv$fator_vencimento$valor$carteira$agencia$campolivre";

$dadosboleto["codigo_barras"] = str_replace($dv, digitoVerificador_barra($linha), $linha);
...
function monta_linha_digitavel($linha) {
	...
	$campo4 = digitoVerificador_barra($linha);		
...
function digitoVerificador_barra($numero) {	
	$soma = modulo_11CodBarras($numero);   	
	$digito = 11 - ($soma % 11);	
...
function modulo_11CodBarras($num, $base=9, $r=0)  {  
  $soma = 0;
  $fator = 1;
  ...
  for ($i = strlen($num); $i > 0; $i--) {  	
	if($i != 5){
		...
		//pr($i . ' - ' . $fator . ' - ' . $soma . ' -- ' . substr($num,$i-1,1));
	}
  }
...
function digitoVerificador_nossonumeroOld($numero) {
	

	$resto2 = modulo_11($numero, 9, 2);
...
function modulo_11_CodBarras($num, $base=9, $r=0) {
	$soma = 0;
	$fator = 2; 
	for ($i = strlen($num); $i > 0; $i--) {
		$numeros[$i] = substr($num,$i-1,1);
