...
// Codigo referente ao tipo de cobran�a: "1" - UNICRED
$tipo_cobranca = 1;
...
$campolivre = $agencia . str_pad($conta . $conta_dv,10,0,STR_PAD_LEFT);
...
//$linha = "$codigobanco$digitoBc$nummoeda$dv$fator_vencimento$valor$campolivre_dv";

$linha  = $codigobanco;
$linha .= 9;
$linha .= '*';
$linha .= $fator_vencimento;
$linha .= $valor;

$campolivre .= $nossonumero_dv;

$linha .= $campolivre;
$dv = digitoVerificador_barra($linha);

$linha = str_replace('*',$dv,$linha);
...
$dadosboleto["codigo_banco_com_dv"] = $codigobanco;

function digitoVerificador_nossonumero($numero) {
	$resto2 = modulo_11($numero, 9, 1);
	// esta rotina sofrer algumas altera��es para ajustar no layout do UNICRED
	 $digito = 11 - $resto2;
     if ($digito > 9 ) {
        $dv = 0;
     } else {
...
function digitoVerificador_campolivre($numero) {
     $resto2 = modulo_11($numero, 9, 1);
// esta rotina sofreu algumas altera��es para ajustar no layout do UNICRED
	if ($resto2 <=1){
		$dv = 0;
	}else{
		$dv = 11 - $resto2;
...
function digitoVerificador_barra($numero) {  
  $resto2 = modulo_11CodBarras($numero, 10);    
  $resto = 11 - $resto2;  
...
function modulo_11CodBarras($num, $base=10, $r=0)  {
