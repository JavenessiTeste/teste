...
if($numRegistro){
	...
	if($_SESSION['perfilOperador'] == 'BENEFICIARIO' || $_SESSION['perfilOperador'] == 'BENEFICIARIO_CPF'){
		$query.= " And Ps1020.Codigo_Associado = " . aspas($_SESSION['codigoIdentificacao']);		
	}elseif($_SESSION['perfilOperador'] == 'EMPRESA' || $_SESSION['perfilOperador'] == 'EMPRESA_CNPJ'){
		$query.= " And Ps1020.Codigo_Empresa = " . aspas($_SESSION['codigoIdentificacao']);		
	}elseif($codAux != ''){
		$query.= " And Ps1020.Codigo_Associado = " . aspas($codAux);				
	}elseif($_GET['especifico'] != 'SIM'){			
		header("Location: https://cartaodemaisdescontos.com.br/AliancaAppNet2/ServidorAl2/EstruturaEspecifica/EstruturaEspecifica/boletoSemParametro.php");
		return false;
	}
}else{
	exit;
...
if($diasAtrazo > 0){
	...
	if($rowPs1020->IDENTIFICACAO_GERACAO == 'SEG-VIA-CART'){
		$dadosboleto["instrucoes4"] = "Apos vencimento cobrar multa de 2% ";
		$dadosboleto["instrucoes3"] = "Apos vencimento cobrar mora diaria de 0,333% ";
	}else{
		$dadosboleto["instrucoes3"] = "Apos vencimento cobrar mora diaria de R$ " . toMoeda(($valor_boleto * $result[0]['PERCENTUAL_MORA_DIARIA']), false, false);
		$dadosboleto["instrucoes4"] = "Apos vencimento cobrar multa de R$ "       . toMoeda(($valor_boleto * $result[0]['PERCENTUAL_MULTA_PADRAO']), false, false);
	}
}
...
$dadosboleto["carteira_descricao"] = "COBRANCA SIMPLES";  // Descri��o da Carteira
...
$dadosboleto["mensagemTopUm"] = 'Imprima em impressora jato de tinta (ink jet) ou laser em qualidade normal ou alta (Nao use modo economico).';
$dadosboleto["mensagemTopDois"] = 'Utilize folha A4 (210 x 297 mm) ou Carta (216 x 279 mm) e margens minimas a esquerda e a direita do formulario.';
$dadosboleto["mensagemTopTres"] = 'Corte na linha indicada. Nao rasure, risque, fure ou dobre a regiao onde se encontra o código de barras.';
...
if($rowPs1020->CODIGO_SMART == '3423' && $rowPs1020->CODIGO_ASSOCIADO != ''){
	...
	$dadosboleto["mensagemTopTres"] = 'PRÓXIMO REAJUSTE: ' . $mesAniversario . '/' . $anoVencimento;
