...
$valida_nova_data = ( $Atualizado && ( time() > mktime(0,0,0,$mes_v,$dia_v,$ano_v ) ) );//&& ( $dif_dias > 2592000 ) );
...
$dadosboleto["instrucoes3"] = $valida_nova_data ? "ESTE BOLETO DESTINA-SE A PAGAMENTO EXCLUSIVO ".date('d/m/Y') : "NÃO RECEBER APÓS 30 DIAS ";
$dadosboleto["instrucoes4"] = $valida_nova_data ? '' : "APÓS O VENCIMENTO COBRAR MULTA DE 1%";
*/
...
$dadosboleto["instrucoes1"] = $valida_nova_data ? 'ESTE BOLETO DESTINA-SE A PAGAMENTO EXCLUSIVO '.date('d/m/Y') : 'APÓS O VENC., PAGÁVEL SOMENTE NO BANCO (SANTANDER, ITAÚ), COM MULTA DE ';
$dadosboleto["instrucoes2"] = $valida_nova_data ? '' : '2% SOBRE O VALOR DA MENSALIDADE E MORA DE 0,033% POR DIA DE ATRASO.';
$dadosboleto["instrucoes3"] = $valida_nova_data ? '' : 'ESTA DATA NÃO EXIME O CANCELAMENTO DO CONTRATO EM CASO DE';
$dadosboleto["instrucoes4"] = $valida_nova_data ? '' : 'ATRASOS SUP A 60 DIAS CONSECUTIVOS OU NÃO NOS ULTIM. 12 MESES';
...
$dadosboleto["carteira"] = $rowPs7300->CODIGO_CARTEIRA ? $rowPs7300->CODIGO_CARTEIRA : '102';  // Cobrança Simples - SEM Registro
